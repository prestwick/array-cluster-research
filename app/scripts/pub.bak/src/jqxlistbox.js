    ...
    $.extend($.jqx._jqxListBox.prototype, {
        ...
        render: function () {
            ...
            var listBoxStructure = $("<div style='-webkit-appearance: none; background: transparent; outline: none; width:100%; height: 100%; align:left; border: 0px; padding: 0px; margin: 0px; left: 0px; top: 0px; valign:top; position: relative;'>" +
                ...
                "<div id='verticalScrollBar" + self.element.id + "' style='visibility: inherit; align:left; valign:top; left: 0px; top: 0px; position: absolute;'/>" +
                "<div id='horizontalScrollBar" + self.element.id + "' style='visibility: inherit; align:left; valign:top; left: 0px; top: 0px; position: absolute;'/>" +
            ...
            self.host.append(listBoxStructure);
            self.filter = self.host.find("#filter" + self.element.id);
            self.filterInput = self.filter.find('input');
            self.filterInput.attr('placeholder', self.filterPlaceHolder);
            self.filterInput.addClass(self.toThemeProperty('jqx-widget jqx-input jqx-rc-all'));
            ...
            var verticalScrollBar = self.host.find("#verticalScrollBar" + self.element.id);
            ...
            var horizontalScrollBar = self.host.find("#horizontalScrollBar" + self.element.id);
            ...
            self.content = self.host.find("#listBoxContent");
            self.content[0].id = 'listBoxContent' + self.element.id;
            self.bottomRight = self.host.find("#bottomRight").addClass(self.toThemeProperty('jqx-listbox-bottomright')).addClass(self.toThemeProperty('jqx-scrollbar-state-normal'));
        ...
        _addItems: function (refreshUIItems) {
            ...
            if (refreshUIItems == false) {
                var virtualSize = me._calculateVirtualSize();
            ...
            if (me.itemswrapper == null) {
                ...
                me.itemswrapper.height(2 * me.host.height());
                me.content.append(me.itemswrapper);
            }
            ...
            var virtualWidth = virtualSize.width;
            me.virtualSize = virtualSize;
            me.itemswrapper.width(Math.max(me.host.width(), 17 + virtualSize.width));
            var startIndex = 0;
            ...
            for (var virtualItemIndex = startIndex; virtualItemIndex < me.virtualItemsCount; virtualItemIndex++) {
                ...
                html += "<span" + oldIEAttribute + " style='-ms-touch-action: none;'></span></div>"
            }
        ...
        unselectIndex: function (index, render) {
            ...
            if (!hasIndexes) {
                this.selectedValue = null;
                this.selectedIndex = -1;
                if (this.selectedValues[this.getVisibleItem(index).value]) {
                    this.selectedValues[this.getVisibleItem(index).value] = null;
                }
            }
