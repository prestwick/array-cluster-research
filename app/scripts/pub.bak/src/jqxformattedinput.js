...
(function ($) {
    ...
    $.extend($.jqx._jqxFormattedInput.prototype, {
        ...
        render: function () {
            ...
            that._number = new that.goog.math.Long.fromString((that.value).toString(), that._radixNumber);
                    ...
                    $.each(children, function (index) {
                        ...
                        if (this.nodeName.toLowerCase() === 'div') {
                            ...
                            that.appendSpinButtons = function (child) {
                                ...
                                that._spinButtonsStepLong = new that.goog.math.Long.fromNumber(that.spinButtonsStep);
                            };

                            var appendAddon = function (child) {
                                that._addon = $(child);
                                that._addon.addClass(that.toThemeProperty('jqx-formatted-input-addon jqx-combobox-arrow-normal'));
                                if (!that._arrow) {
            ...
            if (that.value !== '' && that.value !== null) {
                ...
                if (that._addon) {
                    that._addon.addClass(that.toThemeProperty('jqx-fill-state-disabled'));
                }
            }

            if (that._radixNumber !== 10 && that.min.toString() === '-9223372036854775808') {
                that._minLong = new that.goog.math.Long.fromNumber(that.min);
            } else {
                that._setMinMax('min');
            }
            if (that._radixNumber !== 10 && that.max.toString() === '9223372036854775807') {
                that._maxLong = new that.goog.math.Long.fromNumber(that.max);
            } else {
                that._setMinMax('max');
            }
        },
        ...
        propertyChangedHandler: function (object, key, oldvalue, value) {
            ...
            if (oldvalue !== value && key === 'spinButtonsStep') {
                object._spinButtonsStepLong = new object.goog.math.Long.fromNumber(value);
            }
        ...
        val: function (value) {
            ...
            if (value && !(typeof value === 'object' && $.isEmptyObject(value) === true) && value !== 'binary' && value !== 'octal' && value !== 'decimal' && value !== 'exponential' && value !== 'scientific' && value !== 'hexadecimal') {
                ...
                if (value && !(typeof value === 'object' && $.isEmptyObject(value) === true)) {
                    if (value === 'exponential' || value === 'scientific') {
        ...
        _changeRadix: function (radix) {
            ...
            var newValue = that._number.toString(newRadix);
        ...
        keydown: function (e) {
            ...
            if (e.ctrlKey === true && keyCode === 67) { // Ctrl + C (Copy)
                return;
            }
        ...
        keyup: function (e) {
            ...
            if (that.element.value !== '') {
                ...
                if (that._addon) {
                    that._addon.removeClass(that.toThemeProperty('jqx-fill-state-disabled'));
                }
            } else {
        ...
        _validateValue: function (value, set) {
            if (value !== '') {
                ...
                var numberToValidate = new that.goog.math.Long.fromString((value).toString(), that._radixNumber);
        ...
        _setMinMax: function (key) {
            var that = this;
            that['_' + key + 'Long'] = new that.goog.math.Long.fromString((that[key]).toString(), that._radixNumber);
        },
        ...
        _getDecimalNotation: function (outputNotation) {
            ...
            function defaultToExponential(defaultValue) {
                ...
                return sign + '' + defaultValue.charAt(0) + '.' + defaultValue.slice(1, defaultValue.length) + 'e+' + power;
            }
        ...
        _incrementOrDecrement: function (func) {
            ...
            if (that._number.toString(that._radixNumber) !== that.element.value) {
                that._number = new that.goog.math.Long.fromString(that.element.value, that._radixNumber);
            }
        ...
        _Long: function () {
            ...
            that.goog = new Object();
            var goog = that.goog;
            goog.math = new Object();
            goog.math.Long = new Object();

            goog.math.Long = function (low, high) {
                this.lowBits = low | 0;  // force into 32 signed bits.
                this.highBits = high | 0;  // force into 32 signed bits.
            };

            goog.math.Long.IntCache = {};

            goog.math.Long.fromInt = function (value) {
                if (-128 <= value && value < 128) {
                    var cachedObj = goog.math.Long.IntCache[value];
                    if (cachedObj) {
                        return cachedObj;
                    }
                }
                ...
                if (-128 <= value && value < 128) {
                    goog.math.Long.IntCache[value] = obj;
                }
                return obj;
            };
        ...
        (value / goog.math.Long.TWO_PWR_32_DBL_) | 0);
            ...
            goog.math.Long.fromString = function (str, optRadix) {
                if (str.length === 0) {
    ...
    goog.math.Long.fromBits(0xFFFFFFFF | 0, 0x7FFFFFFF | 0);
            ...
            goog.math.Long.prototype.toNumber = function () {
                return this.highBits * goog.math.Long.TWO_PWR_32_DBL_ +
         this.getLowBitsUnsigned();
            };

            goog.math.Long.prototype.toString = function (optRadix) {
                var radix = optRadix || 10;
                if (radix < 2 || 36 < radix) {
                    throw new Error('radix out of range: ' + radix);
                }
      ...
      this.lowBits : goog.math.Long.TWO_PWR_32_DBL_ + this.lowBits;
            };
      ...
      other.lessThan(goog.math.Long.TWO_PWR_24_)) {
        ...
        other.equals(goog.math.Long.NEG_ONE)) {
            ...
            goog.math.Long.prototype.not = function () {
                return goog.math.Long.fromBits(~this.lowBits, ~this.highBits);
            };

            goog.math.Long.prototype.and = function (other) {
                return goog.math.Long.fromBits(this.lowBits & other.lowBits,
